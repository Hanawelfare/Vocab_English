...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function renderVocabManager() {
    // Generate category tab filters dynamically
    const categories = ["All", ...new Set(vocabList.map(item => item.category))];
    const tabsContainer = document.getElementById("vocab-manager-tabs");
    tabsContainer.innerHTML = "";

    categories.forEach(cat => {
      const tab = document.createElement("button");
      tab.className = `filter-tab ${managerCategoryFilter === cat ? 'active' : ''}`;
      tab.textContent = cat;
      tab.addEventListener("click", () => {
        playSound("click");
        managerCategoryFilter = cat;
        renderVocabListTable();
        renderVocabManager(); // Redraw tabs
      });
