    ...
    for (let i = 0; i < pendingWords.length; i += batchSize) {
      ...
      const promises = batch.map(async (wordObj) => {
        try {
          ...
          if (res) {
            // Robust parsing of response keys
            const ipaValue = res.ipa || res.pronunciation || res.read || "";
            const sentenceValue = res.sentence || res.example || res.exampleSentence || res.englishSentence || "";
            const sentenceThValue = res.sentenceTranslation || res.sentence_translation || res.translation || res.thai || res.thaiTranslation || res.thai_translation || "";
            
            if (sentenceValue) {
              const idx = vocabList.findIndex(item => item.id === wordObj.id);
              if (idx !== -1) {
                vocabList[idx].ipa = ipaValue;
                vocabList[idx].sentence = sentenceValue;
                vocabList[idx].sentenceTranslation = sentenceThValue;
                actualSuccesses++;
              }
            }
          }
...
Return ONLY a valid JSON object in this exact format, with no markdown code blocks, no backticks, and no extra text:
{
  ...
  "sentenceTranslation": "แปลประโยคภาษาไทย"
    ...
    // Strip potential markdown wrappers using regex
