...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function initData() {
    ...
    if (storedVocab) {
      ...
      // Auto-update translations to corrected spelling from DEFAULT_VOCABULARY
      let updated = false;
      const defaultMap = new Map(DEFAULT_VOCABULARY.map(item => [item.id, item]));
      vocabList.forEach(item => {
        const defaultItem = defaultMap.get(item.id);
        if (defaultItem && defaultItem.translation !== item.translation) {
          item.translation = defaultItem.translation;
          updated = true;
        }
      });
  ...
  navItems.forEach(item => {
    item.addEventListener("click", () => {
      ...
      if (view === "study" && (!studyDeck || studyDeck.length === 0)) {
        switchView("dashboard");
        showToast("กรุณาเลือกหมวดหมู่คำศัพท์หรือเริ่มทบทวนจากหน้าแรกก่อนเล่นเกมค่ะ", "info");
        return;
      }
      switchView(view);
    });
  ...
  async function generateAISentences() {
    ...
    const batchSize = 20; // Batch 20 words in a single prompt to stay within rate limits!
    ...
    for (let i = 0; i < pendingWords.length; i += batchSize) {
      ...
      try {
        const resList = await callGeminiAPIBatch(batch);
        if (resList && Array.isArray(resList)) {
          resList.forEach(res => {
            const wordLower = (res.word || "").trim().toLowerCase();
            const matchedWord = batch.find(item => item.word.trim().toLowerCase() === wordLower);
            
            if (matchedWord && res.sentence) {
              const idx = vocabList.findIndex(item => item.id === matchedWord.id);
              if (idx !== -1) {
                vocabList[idx].ipa = res.ipa || res.pronunciation || res.read || "";
                vocabList[idx].sentence = res.sentence || res.example || res.exampleSentence || "";
                vocabList[idx].sentenceTranslation = res.sentenceTranslation || res.sentence_translation || res.translation || res.thai || "";
                actualSuccesses++;
              }
            }
          });
        ...
        console.error("Gemini API Error for batch " + (i / batchSize + 1), err);
      ...
      // 15 seconds cooldown to strictly respect the strict 5 RPM free limit
    ...
    if (actualSuccesses > 0) {
      showToast(`เสร็จสิ้น! สร้างประโยคตัวอย่างสำเร็จ ${actualSuccesses} คำ`, "success");
    } else {
      let failMsg = "ล้มเหลว! ไม่สามารถสร้างประโยคตัวอย่างได้";
      if (lastErrorMessage) {
        failMsg += ` (สาเหตุ: ${lastErrorMessage})`;
      }
  ...
  async function callGeminiAPIBatch(wordsBatch) {
    const url = `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${geminiApiKey}`;
    
    const wordsDetails = wordsBatch.map(item => `Word: "${item.word}", Meaning: "${item.translation}"`).join("\n");
    
    const prompt = `Translate the following English words to Thai and generate:
1. The IPA pronunciation guide.
2. A short English example sentence using the word.
3. The Thai translation of that example sentence.

Here is the list of words:
${wordsDetails}

Return ONLY a valid JSON array of objects in this exact format, with no markdown code blocks, no backticks, and no extra text:
[
  {
    "word": "word here",
    "ipa": "/pronounced/",
    ...
    try {
      ...
      throw new Error(`การเชื่อมต่อเครือข่ายล้มเหลว (Network error: ${fetchErr.message})`);
    }
    ...
    if (!data.candidates || data.candidates.length === 0 || !data.candidates[0].content || !data.candidates[0].content.parts || data.candidates[0].content.parts.length === 0) {
      throw new Error("ไม่มีคำตอบจาก AI");
    }
